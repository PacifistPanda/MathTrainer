"""
Math Trainer — Entraînement Maths (version française)
...
class MathTrainer:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Entraînement Maths")
    ...
    def _build_ui(self):
        ...
        self.mode_label = tk.Label(self.mode_row, text="Mode :", font=FONT)
        ...
        self.digits_label = tk.Label(self.digits_row, text="Chiffres :", font=FONT)
        ...
        self.time_label = tk.Label(self.settings_row, text="Temps :", font=FONT)
        ...
        self.count_label = tk.Label(self.settings_row, text="Nombre :", font=FONT)
        ...
        self.score_label = ttk.Label(self.bottom, text="Score : 0 / 0", style="Score.TLabel")
        self.score_label.pack(side=tk.LEFT)

        best = self._all_time_best()
        self.best_label = ttk.Label(self.bottom, text=f"Meilleur : {best:.0f}%", style="Best.TLabel")
    ...
    def toggle_pause(self):
        ...
        if self.paused:
            ...
            self.paused = True
            self.pause_btn.config(text="▶ Reprendre", bg=self.t["wrong"], activebackground=self.t["wrong"])
            self.answer_entry.config(state="disabled")
            if self.timer_id:
    ...
    def check_answer(self, event=None):
        ...
        if user_answer == self.current_answer:
            ...
            self.score += 1
            self._flash(self.t["correct"], "✓ Correct !")
        else:
    ...
    def _timeout(self):
        ...
        self.timer_id = None
        self._flash(self.t["wrong"], f"✗ Temps écoulé ! Réponse : {self.current_answer}")
        self.waiting_for_next = True
        self.answer_entry.config(state="disabled")
    ...
    def finish_game(self):
        ...
        d_label = {0: "aléatoire", 1: "1 chiffre", 2: "2 chiffres", 3: "3 chiffres", 4: "4 chiffres"}[self.digits.get()]
        ...
        if stats["today_count"] and stats["today_count"] > 1:
            dh += f"  ({stats['today_count']} parties aujourd'hui)"
        lines.append(f"{'Record du jour :':<{L}}{dh}")

        ah = f"{stats['all_time_high']:.0f}%" if stats["all_time_high"] is not None else "---"
        lines.append(f"{'Record absolu :':<{L}}{ah}")

        if stats["vs_last"] is not None:
            d = accuracy - stats["vs_last"]
            vs = f"↑ +{d:.0f}% de mieux !" if d > 0 else (f"↓ {d:.0f}% de moins" if d < 0 else "Identique")
        else:
            vs = "Première partie !"
        lines.append(f"{'vs Partie précéd. :':<{L}}{vs}")

        fg = self._fmt_time(stats["fastest_game"]) if stats["fastest_game"] is not None else "---"
        lines.append(f"{'Partie la + rapide :':<{L}}{fg}")

        sl = self._fmt_time(stats["slowest_game"]) if stats["slowest_game"] is not None else "---"
        lines.append(f"{'Partie la + lente :':<{L}}{sl}")

        parts = []
        if stats["fastest_solve_today"] is not None:
            parts.append(f"{stats['fastest_solve_today']:.1f}s aujourd'hui")
        if stats["fastest_solve_all"] is not None:
            parts.append(f"{stats['fastest_solve_all']:.1f}s total")
        lines.append(f"{'Résolution la + rapide :':<{L}}{' | '.join(parts) if parts else '---'}")
        ...
        if new_alltime:
            self.score_label.config(text=f"🔥 NOUVEAU RECORD ABSOLU ! {self.score}/{self.total} ({accuracy:.0f}%) 🔥", foreground=t["gold"])
        elif new_daily:
            self.score_label.config(text=f"🔥 NOUVEAU RECORD DU JOUR ! {self.score}/{self.total} ({accuracy:.0f}%) 🔥", foreground=t["correct"])
        else:
            self.score_label.config(text=f"Score : {self.score} / {self.total} ({accuracy:.0f}%)", foreground=t["score"])

        self.best_label.config(text=f"Meilleur : {self._all_time_best():.0f}%")
