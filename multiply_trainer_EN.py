"""
Math Trainer — A competitive mental arithmetic training application.
...
class MathTrainer:
    ...
    def _build_ui(self):
        ...
        self.time_label = tk.Label(self.settings_row, text="Time:", font=FONT)
        ...
        self.count_label = tk.Label(self.settings_row, text="Count:", font=FONT)
    ...
    def _timeout(self):
        ...
        self.timer_id = None
        self._flash(self.t["wrong"], f"✗ Time's up! Answer: {self.current_answer}")
        self.waiting_for_next = True
        self.answer_entry.config(state="disabled")
    ...
    def finish_game(self):
        ...
        if stats["today_count"] and stats["today_count"] > 1:
            dh += f"  ({stats['today_count']} games today)"
        lines.append(f"{'Daily High:':<{L}}{dh}")

        ah = f"{stats['all_time_high']:.0f}%" if stats["all_time_high"] is not None else "---"
        lines.append(f"{'All-Time High:':<{L}}{ah}")

        if stats["vs_last"] is not None:
            d = accuracy - stats["vs_last"]
            vs = f"↑ +{d:.0f}% Better!" if d > 0 else (f"↓ {d:.0f}% Worse" if d < 0 else "Same")
        else:
            vs = "First game!"
        lines.append(f"{'vs Last Game:':<{L}}{vs}")

        fg = self._fmt_time(stats["fastest_game"]) if stats["fastest_game"] is not None else "---"
        lines.append(f"{'Fastest Game:':<{L}}{fg}")

        sl = self._fmt_time(stats["slowest_game"]) if stats["slowest_game"] is not None else "---"
        lines.append(f"{'Slowest Game:':<{L}}{sl}")

        parts = []
        if stats["fastest_solve_today"] is not None:
            parts.append(f"{stats['fastest_solve_today']:.1f}s today")
        if stats["fastest_solve_all"] is not None:
            parts.append(f"{stats['fastest_solve_all']:.1f}s all-time")
        lines.append(f"{'Fastest Solve:':<{L}}{' | '.join(parts) if parts else '---'}")
        ...
        if new_alltime:
            self.score_label.config(text=f"🔥 NEW ALL-TIME BEST! {self.score}/{self.total} ({accuracy:.0f}%) 🔥", foreground=t["gold"])
        elif new_daily:
            self.score_label.config(text=f"🔥 NEW DAILY BEST! {self.score}/{self.total} ({accuracy:.0f}%) 🔥", foreground=t["correct"])
        else:
